...
export default function WorkspacePage() {
  ...
  // The currently generated WSI attention artifact is available
  // for GigaPath-Flash. Do not request it for models without a
  // corresponding generated heatmap.
  const heatmapAvailable =
    selectedModel === "gigapath-flash";
  ...
  // Reset generated overlays whenever the selected slide changes.
  useEffect(() => {
    setShowMask(false);
    setShowHeatmap(false);
  }, [selectedSlide]);
  ...
  async function runMIL() {
    ...
    if (!heatmapAvailable) {
      setError(
        "Attention MIL heatmap is currently available only for GigaPath-Flash."
      );
      return;
    }
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => {
                    if (!heatmapAvailable) {
                      setError(
                        "Attention heatmap is currently available only for GigaPath-Flash."
                      );
                      return;
                    }

                    setShowHeatmap((current) => {
                      const next = !current;
                      if (next) {
                        setShowMask(false);
                      }
                  ...
                  title={
                    heatmapAvailable
                      ? "Show the generated GigaPath-Flash attention heatmap"
                      : "Heatmap currently available only for GigaPath-Flash"
                  }
                  className={`inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1.5 text-[10px] transition ${
                    !heatmapAvailable
