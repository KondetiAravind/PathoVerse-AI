...
export async function runClassification(
  ...
  return request<ClassificationResponse>(
...
export async function runMIL(
  slideId: string,
  model = "gigapath-flash"
): Promise<MILResponse> {
  return request<MILResponse>(
    "/api/analysis/mil",
    {
      method: "POST",
      body: JSON.stringify({
...
export function getHeatmapUrl(
  ...
  return (
    ...
    `${encodeURIComponent(slideId)}/heatmap` +
...
export function getTileImageUrl(
  ...
  return (
    `${API_BASE}/api/slides/` +
    `${encodeURIComponent(slideId)}/tiles/` +
    `${tileId}/image`
  );
}